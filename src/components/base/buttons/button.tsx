...
export const styles = sortCx({
    ...
    sizes: {
        xs: {
            root: [
                "gap-1 rounded-md px-2.5 py-1.5 text-sm font-semibold before:rounded-[7px] data-icon-only:p-2",
        ...
        lg: {
            root: "gap-1.5 rounded-md px-4 py-2.5 text-md font-semibold before:rounded-[7px] data-icon-only:p-3",
            linkRoot: "gap-1.5 *:data-text:underline-offset-4",
        },
        xl: {
            root: "gap-1.5 rounded-md px-4.5 py-3 text-md font-semibold before:rounded-[7px] data-icon-only:p-3.5",
            linkRoot: "gap-1.5 *:data-text:underline-offset-4",
        },
    },
    ...
    colors: {
        primary: {
            root: [
                // bg-brand-solid (lime, high-luminance) is paired with text-primary_on-brand (dark olive),
                // matching tokens.json's background.accent / foreground.on-accent pairing — not text-white,
                // which fails contrast against the light lime fill.
                "bg-brand-solid text-primary_on-brand shadow-xs-skeuomorphic ring-1 ring-transparent ring-inset hover:bg-brand-solid_hover data-loading:bg-brand-solid_hover",
                // Inner border gradient
                "before:absolute before:inset-px before:border before:border-white/12 before:mask-b-from-0%",
                // Icon styles
                "*:data-icon:text-primary_on-brand/60 hover:*:data-icon:text-primary_on-brand/70",
            ].join(" "),
        },
        secondary: {
            root: [
